...
Source0: pluginbox-%{version}.tar.gz
...
%description
Pluginbox is a special Kannel box that sits between bearerbox and smsbox and uses
...
%{__install} -D -m 0640 example/pluginbox.conf.example %{buildroot}%{_sysconfdir}/kannel/pluginbox.conf

strip %{buildroot}%{_sbindir}/pluginbox
...
%attr(0640, kannel, kannel) %config(noreplace) %{_sysconfdir}/kannel/pluginbox.conf
%{_sbindir}/*
...
- First RPM version for Pluginbox
